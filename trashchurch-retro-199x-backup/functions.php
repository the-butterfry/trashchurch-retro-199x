...
/**
 * Optional header-fonts include (safe; only loads if present)
 * - Provides Customizer control, @font-face injection (TTF/WOFF/WOFF2), and live preview.
 */
if ( file_exists( get_template_directory() . '/inc/header-fonts.php' ) ) {
    require_once get_template_directory() . '/inc/header-fonts.php';
}
...
function tr199x_enqueue_classifieds_css() {
    wp_enqueue_style('classifieds-prayer', get_template_directory_uri() . '/assets/css/prayer.css', [], null);
}
add_action('wp_enqueue_scripts', 'tr199x_enqueue_classifieds_css');
...
function tr199x_customize($wp) {
    ...
    $wp->add_section('tr199x_retro', array(
        ...
        'priority'=>30,
        'description'=>__('Toggle maximal 1990s effects (marquee, comet cursor, hit counter, color scheme).','trashchurch-retro-199x')
    ));
...
if ( file_exists( get_template_directory() . '/functions/prayer-functions.php' ) ) {
    require_once get_template_directory() . '/functions/prayer-functions.php';
}
