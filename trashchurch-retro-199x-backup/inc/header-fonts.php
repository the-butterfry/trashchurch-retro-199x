...
/** Build @font-face CSS and :root var for a family slug (Title) */
...
/** Build @font-face CSS for Tagline using its own family name */
function tr199x_build_tagline_font_css( $family_slug ) {
    $map = tr199x_get_header_font_families_map();
    if ( empty($family_slug) || ! isset($map[$family_slug]) ) {
        return '';
    }
    $css = "/* tr199x tagline font: {$family_slug} */\n";
    foreach ( $map[$family_slug]['faces'] as $face ) {
        $srcs = array();
        if ( ! empty($face['srcs']['woff2']) ) $srcs[] = "url('{$face['srcs']['woff2']}') format('woff2')";
        if ( ! empty($face['srcs']['woff']) )  $srcs[] = "url('{$face['srcs']['woff']}') format('woff')";
        ...
        $s = ($face['style'] === 'italic') ? 'italic' : 'normal';
        $src = implode(', ', $srcs);
        $css .= "@font-face{font-family:'TR199X-Tagline';font-style:{$s};font-weight:{$w};font-display:swap;src:{$src};}\n";
    }
    return $css;
}
...
function tr199x_build_header_typography_css() {
    ...
    // Only typographic properties for tagline; color/effects remain in theme CSS
...
function tr199x_header_font_inline_css() {
    ...
    $selected_header = get_theme_mod('tr199x_header_font_family', '');
    if ( $selected_header ) {
        $fcss = tr199x_build_header_font_css( $selected_header );
        if ( $fcss ) $styles .= $fcss . "\n";
    }

    // Tagline font family selection (same picker as title, separate setting)
    $selected_tagline = get_theme_mod('tr199x_header_tagline_font_family', '');
    if ( $selected_tagline ) {
        $gcss = tr199x_build_tagline_font_css( $selected_tagline );
        if ( $gcss ) $styles .= $gcss . "\n";
        // Set CSS variable to use the tagline family; fallback to header variable
        $styles .= ":root{--tr-tagline-font:'TR199X-Tagline', var(--tr-header-font);}\n";
    }
...
function tr199x_customize_header_font( $wp ) {
    ...
    // NEW: Tagline font select (same choices)
    ...
    $wp->add_control('tr199x_header_tagline_font_family', array(
        'label'       => __('Tagline Font','trashchurch-retro-199x'),
        'type'        => 'select',
        'section'     => $section,
        'choices'     => $font_choices,
        'description' => __('Optional: choose a different font for the tagline. Leave blank to inherit Header Font.', 'trashchurch-retro-199x')
    ));
...
function tr199x_customize_preview_assets() {
    ...
    // Live: update tagline font family using same picker, inject @font-face and swap CSS var
    if ( function_exists('wp_add_inline_script') ) {
        wp_add_inline_script(
            'tr199x-customize-preview',
            '(function(api){' .
              'function buildTaglineCss(slug){' .
                'var fam = (window.TR199X_FONTMAP||{})[slug]; if(!fam){return "";}' .
                'var css = "/* tr199x tagline font: "+slug+" */\\n";' .
                'if(Array.isArray(fam.faces)){' .
                  'fam.faces.forEach(function(face){' .
                    'var srcs=[]; if(face.srcs&&face.srcs.woff2){srcs.push("url(\'"+face.srcs.woff2+"\') format(\'woff2\')");}' .
                    'if(face.srcs&&face.srcs.woff){srcs.push("url(\'"+face.srcs.woff+"\') format(\'woff\')");}' .
                    'if(face.srcs&&face.srcs.ttf){srcs.push("url(\'"+face.srcs.ttf+"\') format(\'truetype\')");}' .
                    'if(!srcs.length){return;}' .
                    'var w = parseInt(face.weight||400,10); var s = (face.style==="italic")?"italic":"normal";' .
                    'css += "@font-face{font-family:\'TR199X-Tagline\';font-style:"+s+";font-weight:"+w+";font-display:swap;src:"+srcs.join(", ")+";}\\n";' .
                  '});' .
                '}' .
                'return css;' .
              '}' .
              'function ensureStyleEl(){var el=document.getElementById("tr199x-tagline-font-style"); if(!el){el=document.createElement("style"); el.id="tr199x-tagline-font-style"; document.head.appendChild(el);} return el;}' .
              'api("tr199x_header_tagline_font_family", function(value){' .
                'value.bind(function(v){' .
                  'var el = ensureStyleEl();' .
                  'if(v){ el.textContent = buildTaglineCss(v); document.documentElement.style.setProperty("--tr-tagline-font", "\'TR199X-Tagline\', var(--tr-header-font)"); }' .
                  'else { el.textContent = ""; document.documentElement.style.setProperty("--tr-tagline-font", ""); }' .
                '});' .
              '});' .
            '})(wp.customize);'
        );
